...
function loadSettings() {
  ...
  const savedVolume = localStorage.getItem("archiware_volume")
  const savedBrightness = localStorage.getItem("archiware_brightness")
  ...
  if (savedWallpaper) {
    if (savedWallpaper.startsWith("video:")) {
      const videoUrl = savedWallpaper.substring(6)
      document.querySelectorAll(".liquid-bg").forEach((bg) => {
        bg.innerHTML = `<video autoplay loop muted playsinline style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; z-index: -1;">${'<source src="' + videoUrl + '" type="video/mp4">'}${"</video>"}`
  ...
  if (savedVolume) {
    currentVolume = Number.parseInt(savedVolume)
  ...
  if (savedBrightness) {
    const brightness = Number.parseInt(savedBrightness)
...
document.addEventListener("DOMContentLoaded", () => {
      ...
      if (file) {
        if (file.type.startsWith("video/")) {
          ...
          reader.onload = (e) => {
            ...
            localStorage.setItem("archiware_wallpaper", "video:" + videoUrl)
            showNotification("Fond d'écran vidéo modifié avec succès")
          }
  ...
  if (volumeSlider && volumeValue) {
    volumeSlider.addEventListener("input", (e) => {
      ...
      updateVolume(value)
      localStorage.setItem("archiware_volume", value)
    })
  }
  ...
  if (brightnessSlider && brightnessValue) {
    brightnessSlider.addEventListener("input", (e) => {
      ...
      localStorage.setItem("archiware_brightness", value)
    })
  }
})
...
function showLockscreenPowerMenu() {
  const powerMenu = document.getElementById("lockscreenPowerMenu")
  if (powerMenu) {
    ...
    setTimeout(() => {
      document.addEventListener("click", closeLockscreenPowerMenuOnClickOutside)
    }, 100)
  }
}

function hideLockscreenPowerMenu() {
  const powerMenu = document.getElementById("lockscreenPowerMenu")
  if (powerMenu) {
    ...
    document.removeEventListener("click", closeLockscreenPowerMenuOnClickOutside)
  }
}

function closeLockscreenPowerMenuOnClickOutside(e) {
  const powerMenu = document.getElementById("lockscreenPowerMenu")
  const powerBtn = document.querySelector(".lockscreen-power-btn")

  if (powerMenu && !powerMenu.contains(e.target) && !powerBtn.contains(e.target)) {
    hideLockscreenPowerMenu()
  }
}
